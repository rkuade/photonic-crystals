...
def compute_derivative_matrices(omega, shape, npml, dL, bloch_x=0.0, bloch_y=0.0, bloch_z=0.0):
    ...
    Dxf = createDws('x', 'f', shape, dL, bloch_x=bloch_x, bloch_y=bloch_y, bloch_z=bloch_z)
    Dxb = createDws('x', 'b', shape, dL, bloch_x=bloch_x, bloch_y=bloch_y, bloch_z=bloch_z)
    Dyf = createDws('y', 'f', shape, dL, bloch_x=bloch_x, bloch_y=bloch_y, bloch_z=bloch_z)
    Dyb = createDws('y', 'b', shape, dL, bloch_x=bloch_x, bloch_y=bloch_y, bloch_z=bloch_z)
...
def createDws(component, dir, shape, dL, bloch_x=0.0, bloch_y=0.0, bloch_z=0.0):
...
def make_Dij(D,i,j):
    ij_mat = sp.coo_matrix((np.array([1]),(np.array([i]),np.array([j]))),shape=(3,3),dtype=npa.complex128)
...
class fdfd():
    ...
    def _setup_derivatives(self):
        ...
        derivs = compute_derivative_matrices(self.omega, self.shape, self.npml, self.dL, bloch_x=self.bloch_x, bloch_y=self.bloch_y, bloch_z=self.bloch_z)
...
class fdfd_TEx(fdfd):
    ...
    def _make_A(self, eps_vec):
        ...
        eps_vec_inv = 1 / (eps_vec + 1e-5)


        indices_diag = npa.vstack((npa.arange(self.N), npa.arange(self.N)))

        entries_DxEpsx22,   indices_DxEpsx22   = spsp_mult(self.entries_Dxf, self.indices_Dxf, eps_vec_yy_inv, indices_diag, self.N)
        entires_DxEpsxDx22, indices_DxEpsxDx22 = spsp_mult(entries_DxEpsx22, indices_DxEpsx22, self.entries_Dxb, self.indices_Dxb, self.N)

        entries_DyEpsy22,   indices_DyEpsy22   = spsp_mult(self.entries_Dyf, self.indices_Dyf, eps_vec_xx_inv, indices_diag, self.N)
        entires_DyEpsyDy22, indices_DyEpsyDy22 = spsp_mult(entries_DyEpsy22, indices_DyEpsy22, self.entries_Dyb, self.indices_Dyb, self.N)

        entries_d = 1 / EPSILON_0 * npa.hstack((entires_DxEpsxDx22, entires_DyEpsyDy22))
        indices_d = npa.hstack((indices_DxEpsxDx22, indices_DyEpsyDy22))
    ...
    def _solve_fn(self, eps_vec, entries_a, indices_a, Mz_vec):
        ...
        entries_DxEpsy21,   indices_DxEpsy21   = spsp_mult(self.entries_Dxf, self.indices_Dxf, eps_vec_yy_inv, indices_diag, self.N)
        
        entries_DyEpsx20,   indices_DyEpsx20   = spsp_mult(self.entries_Dyf, self.indices_Dyf, eps_vec_xx_inv, indices_diag, self.N)

        b_vec = -1 / EPSILON_0 * (-sp_mult(entries_DyEpsx20, indices_DyEpsx20, Mz_vec))          # needed so fields are SI units
        H_vec = sp_solve(entries_a, indices_a, b_vec)
        eps_vec_xx, eps_vec_yy = self._grid_average_2d(eps_vec)

        # strip out the x and y components of E and find the Hz component
        Ex_vec = self._Hy_Hz_to_Ex(0*H_vec,H_vec, eps_vec_xx)
        Ey_vec = self._Hx_Hz_to_Ey(0*H_vec,H_vec, eps_vec_yy)

        return Ex_vec, Ey_vec, H_vec
...
class fdfd_TEy(fdfd):
    ...
    def _solve_fn(self, eps_vec, entries_a, indices_a, Mz_vec):
        ...
        b_vec = -1 / EPSILON_0 * (sp_mult(entries_DxEpsy21, indices_DxEpsy21, Mz_vec))          # needed so fields are SI units
        H_vec = sp_solve(entries_a, indices_a, b_vec)
        eps_vec_xx, eps_vec_yy = self._grid_average_2d(eps_vec)
